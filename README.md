...
```
if command -v rocminfo; then export GFX_NAME=$(rocminfo | grep -m 1 -E "gfx[^0]{1}" | sed -e 's/ *Name: *//' | awk '{$1=$1; print}'); else echo "rocminfo missing!"; fi
if [ -z "${GFX_NAME}" ]; then echo 'Error: Could detect GPU!'; else echo "Building for GPU: ${GFX_NAME}"; fi
