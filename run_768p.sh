#!/bin/bash
# 768p (768×512 or 512×768) — Optimal LTX resolution
# 5% fewer tokens than 480p, native 3:2 aspect ratio
# Multi-GPU: diffusion=cuda2, te=cuda3, vae=cuda1
# Usage: ./run_768p.sh [prompt] [frames] [seed] [output] [portrait] [threads] [vae] [steps]

PROMPT="${1:-Cinematic high quality video, smooth motion, consistent details.}"
FRAMES="${2:-129}"
SEED="${3:-42}"
OUTPUT="${4:-output/768p_f${FRAMES}_s${SEED}.webm}"
PORTRAIT="${5:-false}"
THREADS="${6:-14}"
VAE="${7:-gpu}"
STEPS="${8:-8}"
