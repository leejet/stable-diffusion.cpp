...
public:
    ...
    bool init(const sd_ctx_params_t* sd_ctx_params) {
        ...
        // Cross-stage / layer-streaming offload modes need params on CPU so they
        // can be moved into VRAM on demand. Express that as an override on the
        // backend manager's params_assignment so the rest of init() can keep
        // using the standard params_backend_for(MODULE) lookup. Doesn't touch
        // any module the user already pinned to a non-default backend via
        // --params-backend (since force_module_params_backend writes per-module
        // entries, leaving the default fall-through intact for everything else).
        if (offload_config.mode != SD_OFFLOAD_NONE) {
            if (offload_config.offload_cond_stage) {
                backend_manager.force_module_params_backend(SDBackendModule::TE, "cpu");
            }
            // Diffusion CPU backend is needed even in cond_only mode so we can
            // temporarily swap it out while loading cond_stage to GPU.
            backend_manager.force_module_params_backend(SDBackendModule::DIFFUSION, "cpu");
        }
        ...
        // the two phases is pure win.
        if (offload_config.mode == SD_OFFLOAD_LAYER_STREAMING) {
            backend_manager.force_module_params_backend(SDBackendModule::VAE, "cpu");
        }
        ...
        cond_stage_on_cpu_only = sd_ctx_params->keep_clip_on_cpu;  // Track for offload decisions
        vae_on_cpu_only        = sd_ctx_params->keep_vae_on_cpu;
    ...
    // VAE was constructed with a CPU-pinned twin (force_module_params_backend(VAE,
    // "cpu") above, which we apply under SD_OFFLOAD_LAYER_STREAMING).
