...
struct LTXVModel : public DiffusionModel {
    ...
    std::string get_desc() override { return ltxv.get_desc(); }
    void alloc_params_buffer() override { ltxv.alloc_params_buffer(); }
    void free_params_buffer() override { ltxv.free_params_buffer(); }
    void free_compute_buffer() override { ltxv.free_compute_buffer(); }
    void get_param_tensors(std::map<std::string, ggml_tensor*>& tensors) override {
        ltxv.get_param_tensors(tensors, prefix);
    }
    size_t get_params_buffer_size() override { return ltxv.get_params_buffer_size(); }
    void set_weight_adapter(const std::shared_ptr<WeightAdapter>& adapter) override {
        ltxv.set_weight_adapter(adapter);
    }
    int64_t get_adm_in_channels() override { return 2048; }
    void set_flash_attention_enabled(bool enabled) override {
        ltxv.set_flash_attention_enabled(enabled);
    }
    void set_circular_axes(bool circular_x, bool circular_y) override {
        ltxv.set_circular_axes(circular_x, circular_y);
    }
    ...
    sd::Tensor<float> compute(int n_threads,
        ...
        GGML_ASSERT(diffusion_params.x != nullptr);
        GGML_ASSERT(diffusion_params.timesteps != nullptr);
        return ltxv.compute(n_threads,
                            *diffusion_params.x,
                            *diffusion_params.timesteps,
                            tensor_or_empty(diffusion_params.context),
                            sd::Tensor<float>());  // encoder attention mask (TODO: plumb through)
    }
};
