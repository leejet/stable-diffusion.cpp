...
public:
    ...
    // Force a module's params onto a specific backend. Used by the dynamic
    // offload modes (layer_streaming etc.) to escalate params onto CPU after
    // init() has already run, without breaking any user-provided --params-backend
    // assignments for other modules.
    void force_module_params_backend(SDBackendModule module, const std::string& backend);
