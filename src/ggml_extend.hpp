...
protected:
    ...
    void init_params(ggml_context* ctx, const String2TensorStorage& tensor_storage_map = {}, const std::string prefix = "") override {
        ...
        for (size_t i = 1; i < out_features_vec.size(); i++) {
            params["weight." + std::to_string(i)] = ggml_new_tensor_2d(ctx, wtype, in_features, out_features_vec[i]);
        }
        if (bias) {
            enum ggml_type bias_wtype = GGML_TYPE_F32;
            params["bias"]            = ggml_new_tensor_1d(ctx, bias_wtype, out_features_vec[0]);
            for (size_t i = 1; i < out_features_vec.size(); i++) {
                params["bias." + std::to_string(i)] = ggml_new_tensor_1d(ctx, bias_wtype, out_features_vec[i]);
            }
        }
    }
...
public:
    ...
    ggml_tensor* forward(GGMLRunnerContext* ctx, ggml_tensor* x) override {
        ...
        ggml_tensor* out = ggml_ext_linear(ctx->ggml_ctx, x, w, b, force_prec_f32, scale);
        for (size_t i = 1; i < out_features_vec.size(); i++) {
            ggml_tensor* wi       = params["weight." + std::to_string(i)];
            ggml_tensor* bi       = bias ? params["bias." + std::to_string(i)] : nullptr;
            ggml_tensor* curr_out = ggml_ext_linear(ctx->ggml_ctx, x, wi, bi, force_prec_f32, scale);
