...
namespace WAN {
    ...
    public:
        WanVAE(bool decode_only = true, SDVersion version = VERSION_WAN2)
            ...
            if (!decode_only) {
                blocks["encoder"] = std::shared_ptr<GGMLBlock>(new Encoder3d(dim, z_dim * 2, input_channels, dim_mult, num_res_blocks, temperal_downsample, wan2_2));
                blocks["conv1"]   = std::shared_ptr<GGMLBlock>(new CausalConv3d(z_dim * 2, z_dim * 2, {1, 1, 1}));
            }
            blocks["decoder"] = std::shared_ptr<GGMLBlock>(new Decoder3d(dec_dim, z_dim, input_channels, dim_mult, num_res_blocks, temperal_upsample, wan2_2));
    ...
    struct WanVAERunner : public VAE {
        ...
        WanVAERunner(ggml_backend_t backend,
            ...
            : decode_only(decode_only), ae(decode_only, version), VAE(version, backend, params_backend) {
