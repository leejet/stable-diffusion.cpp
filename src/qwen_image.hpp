    ...
    public:
        ...
        ggml_tensor* forward(GGMLRunnerContext* ctx,
            ...
            auto patchify_input = [&](ggml_tensor* input) -> ggml_tensor* {
                input = DiT::pad_to_patch_size(ctx, input, params.patch_size, params.patch_size);
                if (!has_time_axis) {
                    return DiT::patchify(ctx->ggml_ctx, input, params.patch_size, params.patch_size);
                }
                if (input->ne[3] == 1) {
                    input = ggml_reshape_4d(ctx->ggml_ctx, input, input->ne[0], input->ne[1], 1, input->ne[2]);
                }
                return DiT::patchify(ctx->ggml_ctx, input, 1, params.patch_size, params.patch_size, N);
            };
            ...
            if (has_time_axis) {
                ...
                int h_len = static_cast<int>((H + pad_h) / params.patch_size);
                int w_len = static_cast<int>((W + pad_w) / params.patch_size);
                out       = DiT::unpatchify(ctx->ggml_ctx, out, T, h_len, w_len, 1, params.patch_size, params.patch_size);
