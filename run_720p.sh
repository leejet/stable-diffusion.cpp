#!/bin/bash
# 720p (1280×704 or 704×1280) video generation
# Multi-GPU: diffusion=cuda2, te=cuda3, vae=cuda1
# NOTE: 720p uses 16x16+t2 for GPU VAE (14x14+t4 OOMs at this resolution)
# Usage: ./run_720p.sh [prompt] [frames] [seed] [output] [portrait] [threads] [vae] [steps]

PROMPT="${1:-Cinematic high quality video, smooth motion, consistent details.}"
FRAMES="${2:-129}"
SEED="${3:-42}"
OUTPUT="${4:-output/720p_f${FRAMES}_s${SEED}.webm}"
PORTRAIT="${5:-false}"
THREADS="${6:-14}"
VAE="${7:-gpu}"
STEPS="${8:-8}"
...
if [ "$VAE" = "cpu" ]; then
  ...
  # 720p: 16x16+t2 safe (14x14+t4 OOMs at this spatial resolution)
  VAE_BACKEND="vae=cuda1"
  VAE_TILES="--vae-tile-size 16x16 --vae-tile-overlap 0.25"
  VAE_TEMP="--extra-tiling-args temporal_tile_frames=2,temporal_tile_overlap=0"
  VAE_THREADS="$THREADS"
  VAE_LABEL="GPU (16x16, temp=2/0 — 720p safe)"
fi

echo "=== 720p Generation ==="
