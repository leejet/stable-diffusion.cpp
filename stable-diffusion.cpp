...
struct ControlNet {
    ...
    bool init(ggml_backend_t backend_, ggml_type wtype_) {
        backend            = backend_;
        wtype              = wtype_;
        memory_buffer_size = 4 * 1024 * 1024;  // 4 MB, for padding
        memory_buffer_size += calculate_mem_size();
        int num_tensors = get_num_tensors();

        LOG_DEBUG("control net params backend buffer size = % 6.2f MB (%i tensors)", memory_buffer_size / (1024.0 * 1024.0), num_tensors);
    ...
    void alloc_params() {
        ...
        input_hint_block.init_params(ctx);
        ...
        // middle_block_out
        middle_block_out_w = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, 1, 1, middle_out_channel, middle_out_channel);
        middle_block_out_b = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, middle_out_channel);
    ...
    void begin(struct ggml_tensor* x,
        ...
        struct ggml_cgraph* gf = build_graph(x, hint, NULL, context, t_emb);
        ...
        LOG_DEBUG("controlnet compute buffer size: %.2f MB (external buffer: %.2f MB)", (compute_memory_buffer_size + control_buffer_size) / 1024.0 / 1024.0, control_buffer_size / 1024.0 / 1024.0);
...
public:
    ...
    ggml_tensor* sample(ggml_context* work_ctx,
        ...
        if(control_hint != NULL) {
            control_net.begin(noised_input, control_hint, c, t_emb);
        }

        diffusion_model.begin(noised_input, c, control_net.controls, t_emb, c_vector);
