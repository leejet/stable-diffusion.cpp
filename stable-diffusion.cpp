        ...
        case DPMPP2S_A:
        {
            ...
            for (int i = 0; i < steps; i++)
            {
                ...
                float sigma_up = std::min(sigmas[i + 1],
                                          std::sqrt(sigmas[i + 1] * sigmas[i + 1] * (sigmas[i] * sigmas[i] - sigmas[i + 1] * sigmas[i + 1]) / (sigmas[i] * sigmas[i])));
                float sigma_down = std::sqrt(sigmas[i + 1] * sigmas[i + 1] - sigma_up * sigma_up);
                auto t_fn = [](float sigma) -> float
                { return -log(sigma); };
                auto sigma_fn = [](float t) -> float
                { return exp(-t); };
...
// Use setter to hanlde purego max args limit less than 9
...
void set_txt2img_cfg_scale(sd_txt2img_options *opt, const char *cfg_scale)
{
    ...
    opt->cfg_scale = strtof(cfg_scale, NULL);
}
...
void set_txt2img_sample_method(sd_txt2img_options *opt, int sample_method)
{
...
void set_txt2img_strength(sd_txt2img_options *opt, float strength)
{
...
void set_img2img_init_img(sd_img2img_options *opt, const uint8_t *init_img)
{
...
void set_img2img_sample_method(sd_img2img_options *opt, int sample_method)
{
    opt->sample_method = sample_method;
}
...
{
    StableDiffusion *s = (StableDiffusion *)sd;
    delete s;
};
...
{
    StableDiffusion *s = (StableDiffusion *)sd;
    std::vector<uint8_t> res = s->txt2img(
